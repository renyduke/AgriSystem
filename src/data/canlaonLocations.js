const canlaonLocations = {
  "Bayog": {
    coordinates: [10.4131, 123.2267],
    sitios: [
      { name: "Purok 1", coordinates: [10.4133, 123.2265] },
      { name: "Purok 2", coordinates: [10.4129, 123.2269] },
      { name: "Sitio A", coordinates: [10.4135, 123.2263] },
      { name: "Sitio B", coordinates: [10.4127, 123.2271] },
    ],
  },
  "Binalbagan": {
    coordinates: [10.3650, 123.2150],
    sitios: [
      { name: "Purok 1", coordinates: [10.3652, 123.2148] },
      { name: "Purok 2", coordinates: [10.3648, 123.2152] },
      { name: "Sitio X", coordinates: [10.3654, 123.2146] },
      { name: "Sitio Y", coordinates: [10.3646, 123.2154] },
    ],
  },
  "Bucalan (East Budsalan)": {
    coordinates: [10.3900, 123.2300],
    sitios: [
      { name: "Purok 1", coordinates: [10.3902, 123.2298] },
      { name: "Purok 2", coordinates: [10.3898, 123.2302] },
      { name: "Sitio C", coordinates: [10.3904, 123.2296] },
      { name: "Sitio D", coordinates: [10.3896, 123.2304] },
    ],
  },
  "Budlasan (West Budsalan)": {
    coordinates: [10.3850, 123.2200],
    sitios: [
      { name: "Purok 1", coordinates: [10.3852, 123.2198] },
      { name: "Purok 2", coordinates: [10.3848, 123.2202] },
      { name: "Sitio E", coordinates: [10.3854, 123.2196] },
      { name: "Sitio F", coordinates: [10.3846, 123.2204] },
    ],
  },
  "Linothangan": {
    coordinates: [10.4000, 123.2400],
    sitios: [
      { name: "Purok 1", coordinates: [10.4002, 123.2398] },
      { name: "Purok 2", coordinates: [10.3998, 123.2402] },
      { name: "Sitio M", coordinates: [10.4004, 123.2396] },
      { name: "Sitio N", coordinates: [10.3996, 123.2404] },
    ],
  },
  "Lumapao": {
    coordinates: [10.4024, 123.2279],
    sitios: [
      { name: "Purok Mangga", coordinates: [10.433504, 123.202569] },
      { name: "Purok Bayabas", coordinates: [10.435774, 123.193808] },
      { name: "Purok Nangka", coordinates: [10.437166, 123.202839] },
      { name: "Purok Tisa", coordinates: [10.430467, 123.210774] },
      { name: "Purok Star Apple", coordinates: [10.433638, 123.215979] },
      { name: "Purok Rambutan", coordinates: [10.426238, 123.219245] },
      { name: "Purok Tambis", coordinates: [10.435983, 123.227080] },
    ],  
  },
  "Malaiba": {
    coordinates: [10.3750, 123.2250],
    sitios: [
      { name: "Purok Rose B", coordinates: [10.411244, 123.211548] },
      { name: "Purok Daisy A(Mapot)", coordinates: [10.423986, 123.177888] },
      { name: "Purok Orchids", coordinates: [10.422908, 123.207812] },
      { name: "Purok Everlasting", coordinates: [10.429368, 123.197373] },
    ],
  },
  "Masulog": {
    coordinates: [10.3950, 123.2350],
    sitios: [
      { name: "Purok 1", coordinates: [10.3952, 123.2348] },
      { name: "Purok 2", coordinates: [10.3948, 123.2352] },
      { name: "Sitio K", coordinates: [10.3954, 123.2346] },
      { name: "Sitio L", coordinates: [10.3946, 123.2354] },
    ],
  },
  "Panubigan": {
    coordinates: [10.3650, 123.2050],
    sitios: [
      { name: "Purok 1", coordinates: [10.3652, 123.2048] },
      { name: "Purok 2", coordinates: [10.3648, 123.2052] },
      { name: "Sitio O", coordinates: [10.3654, 123.2046] },
      { name: "Sitio P", coordinates: [10.3646, 123.2054] },
    ],
  },
  "Mabigo (Pob.)": {
    coordinates: [10.3860, 123.2220],
    sitios: [
      { name: "Purok 1", coordinates: [10.3862, 123.2218] },
      { name: "Purok 2", coordinates: [10.3858, 123.2222] },
      { name: "Sitio Q", coordinates: [10.3864, 123.2216] },
      { name: "Sitio R", coordinates: [10.3856, 123.2224] },
    ],
  },
  "Pula": {
    coordinates: [10.4100, 123.2450],
    sitios: [
      { name: "Purok 1", coordinates: [10.405776, 123.175428] },
      { name: "Purok 2", coordinates: [10.4098, 123.2452] },
      { name: "Sitio S", coordinates: [10.4104, 123.2446] },
      { name: "Sitio T", coordinates: [10.4096, 123.2454] },
    ],
  },
  "Ninoy Aquino": {
    coordinates: [10.4239, 123.2327],
    sitios: [
      { name: "Purok 1", coordinates: [10.4241, 123.2325] },
      { name: "Purok 2", coordinates: [10.4237, 123.2329] },
      { name: "Sitio U", coordinates: [10.4243, 123.2323] },
      { name: "Sitio V", coordinates: [10.4235, 123.2331] },
    ],
  },
};

export default canlaonLocations;