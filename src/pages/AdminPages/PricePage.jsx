...
const PricePage = () => {
  ...
  const processWeeklyData = () => {
    ...
    filteredData.forEach(item => {
      ...
      if (!weeklyGroups[weekKey].commodityPrices[baseName]) {
        weeklyGroups[weekKey].commodityPrices[baseName] = {
          lowest: item.lowest_price || null,
          highest: item.highest_price || null,
          average: item.average_price || null
        };
      }
    });
    return weeklyGroups;
  ...
  const downloadCSV = () => {
    ...
    Object.entries(weeklyData).sort(([a], [b]) => a.localeCompare(b)).forEach(([_, data]) => {
      ...
      vegetableCommodities.forEach((c, i) => {
        const p = data.commodityPrices[getBaseCommodityName(c)];
        csv += `${i + 1},"${formatCommodityName(c)}",${p && p.lowest ? '₱' + p.lowest.toFixed(2) : ''},${p && p.highest ? '₱' + p.highest.toFixed(2) : ''},${p && p.average ? '₱' + p.average.toFixed(2) : ''}\n`;
      });
  ...
  const buildSectionRowsHtml = (commodityList, data) => {
    ...
    commodityList.forEach((commodity, idx) => {
      const prices = data.commodityPrices[getBaseCommodityName(commodity)];
      html += `
        <tr>
          <td>${idx + 1}</td>
    ...
    return (
      <div className={`min-h-screen ${darkMode ? "bg-slate-950" : "bg-gradient-to-br from-green-50 to-blue-50"} flex items-center justify-center transition-colors duration-300`}>
        <div className="text-center">
          ...
          <p className={`mt-4 ${darkMode ? "text-slate-400" : "text-gray-600"}`}>Loading price data...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="overflow-x-auto">
      <table className={`min-w-full divide-y ${darkMode ? "divide-slate-700" : "divide-gray-300"}`}>
        <thead className={darkMode ? "bg-slate-800" : "bg-gray-100"}>
          <tr>
            <th rowSpan={2} className={`px-3 py-3 text-center text-xs font-bold ${darkMode ? "text-slate-300 border-slate-700" : "text-gray-900 border-gray-300"} uppercase border w-12`}>No.</th>
            <th rowSpan={2} className={`px-4 py-3 text-center text-xs font-bold ${darkMode ? "text-slate-300 border-slate-700" : "text-gray-900 border-gray-300"} uppercase border`}>Commodity</th>
            <th colSpan={3} className={`px-4 py-2 text-center text-xs font-bold ${darkMode ? "text-slate-300 border-slate-700" : "text-gray-900 border-gray-300"} uppercase border`}>Prices (Php/Kilograms)</th>
            <th rowSpan={2} className={`px-4 py-3 text-center text-xs font-bold ${darkMode ? "text-slate-300 border-slate-700" : "text-gray-900 border-gray-300"} uppercase border w-28`}>Remarks</th>
            <th rowSpan={2} className={`px-4 py-3 text-center text-xs font-bold ${darkMode ? "text-slate-300 border-slate-700" : "text-gray-900 border-gray-300"} uppercase border w-24`}>Actions</th>
          </tr>
          <tr>
            <th className={`px-4 py-2 text-center text-xs font-bold ${darkMode ? "text-slate-300 border-slate-700" : "text-gray-900 border-gray-300"} border w-28`}>Lowest</th>
            <th className={`px-4 py-2 text-center text-xs font-bold ${darkMode ? "text-slate-300 border-slate-700" : "text-gray-900 border-gray-300"} border w-28`}>Highest</th>
            <th className={`px-4 py-2 text-center text-xs font-bold ${darkMode ? "text-slate-300 border-slate-700" : "text-gray-900 border-gray-300"} border w-28`}>Average</th>
          </tr>
        </thead>
          ...
          {commodityList.map((commodity, idx) => {
            ...
            return (
              <tr key={commodity} className={darkMode ? "hover:bg-slate-800/50" : "hover:bg-gray-50"}>
                ...
                <td className={`px-4 py-2 text-center text-sm font-semibold ${darkMode ? "text-slate-300 border-slate-700" : "text-gray-900 border-gray-300"} border`}>
  ...
  return (
    <div className={`min-h-screen ${darkMode ? "bg-slate-950 text-slate-200" : "bg-gray-50 text-slate-800"} px-6 pt-2 pb-8 w-full font-sans transition-colors duration-300`}>
      {/* Header */}
      <div className="mb-8">
        <h1 className={`text-3xl font-bold ${darkMode ? "text-white" : "text-slate-800"} mb-2`}>Price Monitoring Report</h1>
        <p className={`text-sm ${darkMode ? "text-slate-400" : "text-slate-500"}`}>
          Agricultural Crops Commodity Price Monitoring
        </p>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
            ...
            <div className="flex gap-3 items-end">
              ...
              <button onClick={handlePrint}
                className={`flex-1 px-4 py-3 ${darkMode ? "bg-slate-800 border-slate-700 text-slate-200 hover:bg-slate-700" : "bg-white border-slate-200 text-gray-700 hover:bg-slate-50"} border rounded-xl font-semibold transition-all flex items-center justify-center gap-2 shadow-sm active:scale-95 text-sm`}>
          ...
          <div className={`${darkMode ? "bg-blue-900/20 border-blue-500/50" : "bg-blue-50 border-blue-500"} border-l-4 p-4 rounded-xl`}>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`text-sm ${darkMode ? "text-blue-400/70" : "text-blue-600/70"}`}>Showing weekly price data per commodity</p>
              </div>
            </div>
          </div>
        </div>
